...
const eliminate = (item: GameWord, index: number) => {
  ...
  // 判断游戏是否结束
  // 1. 平民全被淘汰
  const isNoUnEliminatedCivilian =
    unEliminatedCivilianIndexList.value.length === 0;
  // 2. 卧底全被淘汰
  const isNoUnEliminatedSpy = unEliminatedSpyIndexList.value.length === 0;
  // 3. 只剩下一个平民且剩余一个卧底
  const isOnlyCivilianAndSpy =
    unEliminatedCivilianIndexList.value.length === 1 &&
    unEliminatedSpyIndexList.value.length >= 1;

  // 卧底获胜条件：平民全被淘汰 || 只剩下一个平民且剩余一个卧底
  const isSpyWin = isNoUnEliminatedCivilian || isOnlyCivilianAndSpy;
  // 平民获胜条件：卧底全被淘汰
  const isCivilianWin = isNoUnEliminatedSpy;

  const isGameOver = isSpyWin || isCivilianWin;
  if (isGameOver) {
    if (isCivilianWin) {
      // 平民胜利
